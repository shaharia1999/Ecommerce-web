        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
            ...
            <div className="flex flex-wrap gap-2 sm:gap-4">
              {(product.images || [product.mainImg]).slice(0, 5).map((imgSrc, idx) => (
                <div
                  ...
                  className={`cursor-pointer bg-white rounded-lg p-1 shadow border-2 transition-all duration-200 ${
                    mainImage === imgSrc ? 'border-orange-500 scale-105' : 'border-gray-200 hover:border-orange-200'
                  }`}
          ...
          <div className="space-y-4 sm:space-y-6">
            <div>
              ...
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-800 mb-2 mt-2">
                {product.title}
              </h1>
              ...
              {/* Description */}
              <p className="text-gray-600 mb-6 leading-relaxed text-[15px] sm:text-base whitespace-pre-line border-b border-gray-100 pb-6">
                    ...
                    {product.colors.map((color, idx) => (
                      <button
                        ...
                        className={`w-9 h-9 rounded-full border-2 transition-all ${
                          selectedColor === idx ? 'border-orange-500 scale-110 shadow-md' : 'border-transparent shadow-sm'
                        }`}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {product.sizes.map((size) => (
                      <button
                        ...
                        className={`px-5 py-2 border-2 rounded-xl font-bold transition-all ${
                          selectedSize === size
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-8">
                <div className="flex items-center border-2 border-gray-200 rounded-xl bg-white shadow-sm w-[150px]">
                  <button
                    onClick={() => setQuantity(Math.max(1, quantity - 1))}
                    className="px-4 py-3 text-gray-600 hover:text-orange-500 font-bold text-xl cursor-pointer disabled:opacity-30"
                  >   
                    −
                  </button>
                  <span className="px-4 py-3 min-w-[50px] text-center font-bold text-gray-800 border-x-2 border-gray-100">
                ...
                <button
                  ...
                  className="flex-1 bg-green-600 text-white font-bold py-4 px-8 rounded-xl hover:bg-green-700 hover:shadow-lg transition-all disabled:bg-gray-300 cursor-pointer"
                >
                  Buy Now
                </button>
                ...
                <button
                  ...
                  className="flex-1 bg-orange-500 text-white font-bold py-4 px-8 rounded-xl hover:bg-orange-600 hover:shadow-lg transition-all disabled:bg-gray-300 cursor-pointer"
                >
                  Add To Cart
                </button>
              </div>
